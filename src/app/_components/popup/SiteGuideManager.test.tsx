...
describe('SiteGuideManager shared guide event', () => {
  ...
  test('keeps the public fixed controls touch-safe and vertically separated', () => {
    ...
    expect(guideCss).toMatch(/right:\s*max\(1\.25rem,\s*env\(safe-area-inset-right\)\)/);
    expect(guideCss).toMatch(/bottom:\s*calc\([^;]*52px[^;]*env\(safe-area-inset-bottom\)\)/);
    expect(guideCss).toMatch(/width:\s*52px/);
    expect(guideCss).toMatch(/min-height:\s*52px/);
