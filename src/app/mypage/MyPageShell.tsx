...
function MyPageLayoutContent({ children }: MyPageShellProps) {
  ...
  const userInfo = {
    ...
    orders: isOrderCountError ? "error" as const : isOrderCountLoading ? null : orderCount ?? 0,
    reviews: getNumberValue(userData?.reviews),
    coupons: isCouponCountError
      ? "error" as const
      : isCouponCountLoading
        ? null
        : couponStats?.available ?? 0
  };
