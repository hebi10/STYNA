...
라이브 데모: https://hebimall.web.app/

홈 하단의 **포트폴리오용 데모 사이트입니다 → 프로젝트 둘러보기**에서 면접용 추천 동선을 바로 확인할 수 있습니다. 별도 데모 비밀번호를 제공하지 않으며 로그인 화면의 체험 버튼은 서버가 발급한 Custom Token을 사용합니다.

권장 확인 순서는 다음과 같습니다.

1. 홈과 STYLE NOW에서 메인 UI와 콘텐츠 구성을 확인합니다.
2. 상품 목록·상세에서 옵션 선택과 장바구니 진입을 확인합니다.
3. 로그인 화면의 **일반 사용자 체험**으로 장바구니 → 체크아웃 → 주문내역 흐름을 확인합니다.
4. 로그아웃 후 **관리자 페이지 체험**으로 관리자 화면에 진입합니다.
5. 관리자 대시보드 → 상품 → 주문 → 이벤트 → 쿠폰 순서로 운영 구조를 확인합니다.
6. 관리자 데모의 변경 버튼이 비활성화되고 상단에 **관리자 체험 모드 · 조회 전용** 안내가 유지되는지 확인합니다.

관리자 체험은 실제 운영 데이터를 읽지 않고 비식별 샘플 데이터만 표시합니다. 실제 결제 승인·청구와 관리자 데이터 변경은 수행되지 않습니다.
...
| 데모 로그인 | `NEXT_PUBLIC_ENABLE_DEMO_LOGIN=true`일 때 일반 사용자 체험·조회 전용 관리자 페이지 체험 버튼 노출 |
