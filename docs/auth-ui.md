...
- 2026-06-22: `/auth/login?redirect=/orders/checkout`처럼 내부 redirect 쿼리가 있으면 로그인 성공 후 해당 경로를 우선 사용한다. 공개 화면의 하드코딩 회원/관리자 빠른 로그인은 포트폴리오 데모 배포 전용으로 둔다.
...
## 2026-07-27 포트폴리오 데모 로그인

- `/auth/login`은 이메일·비밀번호 로그인, 일반 회원 빠른 로그인, 관리자 빠른 로그인의 세 가지 로그인 경로를 유지한다. 모든 경로는 기존 `login(email, password, rememberMe)` 호출과 same-origin redirect 검증을 사용한다.
- 일반 회원 빠른 로그인과 관리자 빠른 로그인은 `NEXT_PUBLIC_ENABLE_DEMO_LOGIN`이 정확히 `true`일 때만 노출한다. 값이 없거나 `false`이면 이메일·비밀번호 로그인 폼만 보인다.
- 빠른 로그인은 포트폴리오 데모 배포 전용이며, 관리자 권한 판정과 Firebase 보안 규칙을 우회하거나 완화하지 않는다.
