...
- `/auth/login`은 이메일·비밀번호 로그인, 일반 회원 빠른 로그인, 관리자 빠른 로그인의 세 가지 로그인 경로를 유지한다. 모든 경로는 기존 `login(email, password, rememberMe)` 호출을 사용한다.
- 일반 회원 빠른 로그인과 읽기 전용 관리자 빠른 로그인은 `NEXT_PUBLIC_ENABLE_DEMO_LOGIN`이 정확히 `true`일 때만 노출한다. 값이 없거나 `false`이면 이메일·비밀번호 로그인 폼만 보인다.
- 읽기 전용 관리자 계정은 `demo_admin` 사용자 역할과 `demoAdmin` token claim이 모두 있어야 하며, 실제 관리자 role·claim을 갖지 않는다. 관리자 셸은 데모 전용 요약만 렌더링한다.
