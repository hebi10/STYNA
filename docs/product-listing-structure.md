...
- 레거시 `categories` 문서에 `isActive`가 없으면 공개 목록과 상세 모두 활성으로 해석한다. 명시적으로 `false`인 문서만 숨기므로, 목록에서 보이는 `Bags`가 `/categories/bags`에서 404가 되는 불일치를 방지한다.
...
## 2026-08-12 상품·카테고리 운영 데이터 감사

- `node scripts/operational-data-audit.js analyze`는 추천 설정의 중복·3개 초과, 대소문자·별칭 정규화 결과가 같은 카테고리, 계절·상품 유형과 소재가 모순되어 보이는 상품 상세를 읽기 전용으로 점검한다.
- 승인 계획의 자동 작업은 추천 상품 ID를 기존 순서대로 중복 제거한 첫 3개로 제한하고 `maxCount: 3`을 기록하는 것, 중복 카테고리 문서에 누락된 `isActive`를 `true`로 명시하는 것뿐이다.
- 카테고리 통합·이름 변경·비활성화와 상품명·소재 보정은 추정하지 않는다. 문서 ID, 현재 값, 제안 값, 보류 사유를 계획의 수동 검토 항목에 남겨 별도 승인받는다.
- remediation은 계획 파일 SHA-256, 프로젝트 ID, 각 문서의 감사 당시 현재 값을 모두 재검증한 뒤 허용된 필드만 단일 batch로 갱신한다. 하나라도 달라지면 전체를 중단한다.
