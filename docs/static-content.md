...
추천 상품은 `FeaturedProductService`와 `/admin/featured-products`에서 관리한다. 홈은 `STYNA SELECT` 편집형 영역으로, 관리자 설정의 제목·설명·내부 무드 이미지 경로와 선택 순서대로 최대 3개 상품을 표시한다. 설정이 없거나 유효한 공개 상품이 없으면 섹션을 숨긴다.

추천 설정 조회가 실패한 경우에만 홈 공개 추천 결과의 앞 3개를 같은 편집형 구성으로 대체 표시한다. 추천 설정이 비활성 상태이거나 설정 조회는 성공했지만 상품이 없는 경우에는 공개 추천 결과가 있어도 섹션을 숨긴다. 설정 조회와 공개 추천 조회가 모두 실패하거나 대체 상품이 없으면 하나의 재시도 가능한 오류 상태를 표시한다.
