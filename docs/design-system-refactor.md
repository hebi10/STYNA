...
## 2026-07-21 도움말 챗봇 명칭 보정

- 플로팅 UI와 모바일 축약 라벨은 `실시간 상담`·`상담` 대신 `도움말 챗봇`·`챗봇`을 사용한다.
- 입력 UI는 사람 상담 연결이나 응답 시간을 약속하지 않으며, 직접 질문이 별도 지원 티켓으로 저장·전송되지 않는다는 경계를 표시한다.

## 2026-07-21 모바일 상품 필터 밀도 보정

- 상품 목록의 카테고리·정렬·가격 컨트롤은 모바일에서 `aria-expanded`와 필터 영역을 연결한 단일 버튼으로 열고 닫는다. 데스크톱에서는 같은 영역을 항상 표시한다.
- 모바일 장식 통계는 전체 상품 수로 오해하지 않도록 `현재 페이지 상품` 한 줄만 남긴다.
- 필터·정렬·가격·페이지 이동 컨트롤은 최소 44px 터치 높이를 사용한다.

## 2026-07-21 모바일 메뉴 포커스 경계

- 열린 모바일 메뉴는 닫기 버튼과 드로어를 같은 modal dialog 범위로 묶고, 나머지 헤더 컨트롤은 `aria-hidden`과 `inert`로 제외한다.
- Tab과 Shift+Tab은 닫기 버튼부터 드로어의 마지막 컨트롤 사이에서 순환하며, Escape·overlay·메뉴 링크로 닫으면 원래 버튼으로 포커스를 복귀한다.
- 960px 이상으로 전환돼 메뉴가 자동으로 닫힐 때는 숨겨지는 버튼으로 포커스를 강제로 돌리지 않고 body 스크롤만 복원한다.

## 2026-07-27 홈 쇼핑 우선 순서와 포트폴리오 고지 통합

- 홈은 배너, 카테고리, 추천, 신상품, 베스트 랭킹, 세일 상품·이벤트, 서비스 정보, 포트폴리오 데모 순서로 정리한다.
- 분산된 데모·큐레이션·리뷰·혜택 예시는 제거하고, 포트폴리오 고지는 하단 `PortfolioDemoSection` 한 곳에서만 제공한다.
- 포트폴리오의 `구현 범위 자세히 보기`와 고정 쇼핑 안내 버튼은 공유 `styna:open-site-guide` 이벤트로 같은 접근성 팝업을 연다.
