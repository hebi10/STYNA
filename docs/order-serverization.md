...
## 2026-07-10 재고·상품 상태 검증 보강
- 서로 다른 옵션이더라도 같은 `productId`의 수량을 합산해 재고를 상품당 한 번만 차감하고, 취소 시에도 한 번만 복원한다.
- 주문 생성 Function은 `status`가 명시된 상품의 경우 `active` 상태만 허용한다. 상품이 `sizes` 또는 `colors` 배열을 선언하면 요청 옵션도 서버에서 검증한다.
- 구형 상품처럼 상태·옵션 배열이 없는 문서는 기존 구매 흐름을 유지한다.
- `functions/__tests__/httpHandlers.test.ts`에서 합산 재고 부족, 비활성 상품, 선언되지 않은 옵션, 취소 복원 및 구형 상품 호환을 검증한다.

## 2026-07-10 데모 주문·배송지 안내
- checkout은 임시 주소를 생성하지 않고 회원 프로필의 실제 `addresses` 또는 기존 `address` 데이터만 사용한다. 등록된 주소가 없으면 주문을 막고 정보 수정 화면으로 안내한다.
- 실제 PG 연동 전 포트폴리오 범위를 명확히 하기 위해 checkout CTA와 완료 화면에 실제 결제가 없는 데모 주문임을 표시한다.

## 2026-07-10 포인트 사용 한도
- 포인트 사용 한도는 **보유 포인트**와 **쿠폰·배송비 적용 후 포인트 사용 전 결제 예정 금액** 중 작은 값이다. 최소 사용 금액이나 별도 비율 한도는 없다.
- checkout 미리보기(`calculateOrderPreview`)와 주문 생성 Function은 이 기준을 각각 계산·검증한다. 최종 주문 생성 시 서버 계산이 우선한다.

## 2026-07-10 구매 후 CTA 정합성
- 주문 목록과 상세의 배송 조회는 `/orders/delivery?orderId={orderId}`로 연결한다. 외부 택배 실시간 추적은 제공하지 않고, 해당 주문에 등록된 운송장·택배사와 현재 주문 상태만 표시한다.
- 운송장이 없는 배송 주문은 성공한 조회처럼 보이지 않도록 `운송장 미등록`과 현재 주문 상태를 함께 표시한다.
- 고객 주문 취소는 `pending`, `confirmed` 상태에서만 기존 `/api/order` 취소 API로 요청한다. 데모 주문에는 실제 결제·환불이 발생하지 않는다는 안내를 유지한다.
- 주문 화면의 직접 리뷰 작성 CTA는 제거했다. 리뷰는 주문 상품 상세의 리뷰 탭에서 작성하도록 안내한다.
