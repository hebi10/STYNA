...
- 고객 주문 취소는 `pending`, `confirmed` 상태에서만 기존 `/api/order` 취소 API로 요청한다. 주문에는 실제 결제·환불이 발생하지 않는다는 안내를 유지한다.
- 주문 화면의 직접 리뷰 작성 CTA는 제거했다. 리뷰는 주문 상품 상세의 리뷰 탭에서 작성하도록 안내한다.

## 2026-07-13 checkout 배송지 직접 입력

- 저장된 배송지가 없어도 checkout에서 받는 분, 연락처, 우편번호, 주소를 직접 입력해 주문할 수 있다.
